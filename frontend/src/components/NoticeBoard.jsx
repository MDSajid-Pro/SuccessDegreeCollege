...
const NoticeBoard = () => { 
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        // Axios gets the data and parses JSON automatically
        const response = await axios.get('http://localhost:4000/api/notices');
        setNotices(response.data); // Data is always in response.data
        setLoading(false);
      } catch (error) {
  ...
  // Replace your existing formatDate function with this one
const formatDate = (dateString) => {
  // 1. If no date is provided
  if (!dateString) return { month: '---', day: '--', full: 'No Date' };

  // 2. Try to create a Date object
