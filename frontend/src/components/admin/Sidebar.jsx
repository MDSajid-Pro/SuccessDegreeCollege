...
const Sidebar = () => {
  return (
      <div className='flex flex-col border-r border-gray-200 min-h-full pt-6'>
          ...
          <NavLink to='/admin/notice' className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-64 cursor-pointer ${isActive && 'bg-blue-100 border-r-4 border-blue-500 '}`}>
              <img src={assets.add_icon} alt="Logo" className="min-w-4 w-5" />
              <p className='hidden md:inline-block'>Add Notice</p>
          </NavLink>
          <NavLink to='/admin/result' className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-64 cursor-pointer ${isActive && 'bg-blue-100 border-r-4 border-blue-500 '}`}>
              <img src={assets.add_icon} alt="Logo" className="min-w-4 w-5" />
