...
import { 
  Trash2, Edit, Plus, X, LayoutDashboard, Save, 
...
const AddResult = () => {
  ...
  const [formData, setFormData] = useState({
    name: "", regNo: "", course: "", year: "", percentage: "", grade: "", rank: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicateStudent = students.find(s => 
        s.regNo.toLowerCase() === formData.regNo.toLowerCase() && s._id !== currentId
    );
  ...
  const resetForm = () => {
    setFormData({ name: "", regNo: "", course: "", year: "", percentage: "", grade: "", rank: "" });
    setIsEditing(false);
    setShowForm(false);
  };
                ...
                <tbody className="divide-y divide-gray-100">
                    {currentStudents.length > 0 ? currentStudents.map((s) => (
                        <tr key={s._id} className="hover:bg-blue-50/50 transition-colors">
                            <td className="p-4 font-medium text-gray-900 whitespace-nowrap">{s.name}</td>
                            <td className="p-4 text-gray-600 whitespace-nowrap">{s.course}</td>
                            <td className="p-4 font-mono text-sm text-gray-600 whitespace-nowrap">{s.regNo}</td>
                            <td className="p-4 font-bold text-gray-800 whitespace-nowrap">{s.percentage}%</td>
                            <td className="p-4 flex justify-center gap-2 whitespace-nowrap">
                                <button onClick={() => handleEditClick(s)} className="p-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition"><Edit size={16}/></button>
                                <button onClick={() => handleDelete(s._id)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition"><Trash2 size={16}/></button>
                            </td>
                        </tr>
                    )) : (
                        <tr>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <input name="name" value={formData.name} onChange={(e)=>setFormData({...formData, name: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none" required />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-gray-500 uppercase">Reg No</label>
                            <input name="regNo" value={formData.regNo} onChange={(e)=>setFormData({...formData, regNo: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none" required />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <select name="course" value={formData.course} onChange={(e)=>setFormData({...formData, course: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                                <option>---Select Course---</option><option>BCA</option><option>B.Sc</option><option>B.Com</option><option>B.A</option>
                            </select>
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-gray-500 uppercase">Year</label>
                            <select name="year" value={formData.year} onChange={(e)=>setFormData({...formData, year: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                                <option>---Select Year---</option><option>2024-25</option><option>2023-24</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <input type="number" name="percentage" placeholder="85.5" value={formData.percentage} onChange={(e)=>setFormData({...formData, percentage: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none" required />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-gray-500 uppercase">Grade</label>
                            <select name="grade" value={formData.grade} onChange={(e)=>setFormData({...formData, grade: e.target.value})} className="border border-gray-300 p-2.5 rounded-lg w-full focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                                <option>---Select Grade---</option><option>Distinction</option><option>First Class</option><option>Second Class</option><option>Fail</option>
                            </select>
                        </div>
                    </div>

                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold flex justify-center items-center gap-2 transition shadow-lg shadow-blue-200 mt-2">
